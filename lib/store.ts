...
//         set((state: StoreType) => {
//           const existingProduct = state.cartProduct.find((p) => p.id === productId)

//           if (existingProduct) {
//             return {
//               cartProduct: state.cartProduct.map((p) =>
//                 p.id === productId ? { ...p, quantity: Math.max((p?.quantity ?? 1) - 1, 1) } : p,
//               ),
//             }
//           } else {
//             return state
...
export const store = create<StoreType>()(
  persist(
    (set) => ({
      ...
      // 🔧 FIXED: remove item when quantity would go to 0
      decreaseQuantity: (productId: number) => {
        set((state: StoreType) => {
          const existingProduct = state.cartProduct.find((p) => p.id === productId)
          ...
          // If this click would make it 0 → remove from cart entirely
          if (currentQty <= 1) {
            return {
          ...
          return {
            cartProduct: state.cartProduct.map((p) => (p.id === productId ? { ...p, quantity: currentQty - 1 } : p)),
          }
        })
      },
